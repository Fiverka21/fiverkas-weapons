...
public final class ModNetwork {
    ...
    private static void handleSacrilegiousSlamRequest(SacrilegiousSlamRequestPayload payload, IPayloadContext context) {
        context.enqueueWork(() -> {
            ...
            player.getCooldowns().addCooldown(stack, com.fiv.fiverkas_weapons.item.Sacrilegious.SLAM_COOLDOWN_TICKS);
            ...
            try {
                ...
                try {
                    if (listener != null) {
                        serverNetworkClass.getMethod("handleAttackRequest", attackReqClass, net.minecraft.server.MinecraftServer.class, net.minecraft.server.level.ServerPlayer.class, Class.forName("net.minecraft.server.network.ServerGamePacketListenerImpl")).invoke(null, attackReq, player.level().getServer(), player, listener);
                    } else {
                        // fallback to overload without listener if present
                        serverNetworkClass.getMethod("handleAttackRequest", attackReqClass, net.minecraft.server.MinecraftServer.class, net.minecraft.server.level.ServerPlayer.class).invoke(null, attackReq, player.level().getServer(), player);
                    }
