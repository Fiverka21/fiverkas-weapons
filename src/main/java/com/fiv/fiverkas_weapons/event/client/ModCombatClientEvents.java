...
public final class ModCombatClientEvents {
    ...
    private static final Map<Identifier, RenderType> RESILIENCE_OUTLINE_RENDER_TYPES = new ConcurrentHashMap<>();
    ...
    private static class PlayerRenderPreEvent {
        private final Player entity;
        private final PoseStack poseStack;
        private final float partialTick;
        private boolean canceled;

        private PlayerRenderPreEvent(Player entity, PoseStack poseStack, float partialTick) {
            this.entity = entity;
    ...
    private static final class PlayerRenderPostEvent extends PlayerRenderPreEvent {
        private final AvatarRenderer renderer;
        private final int packedLight;

        private PlayerRenderPostEvent(
                Player entity,
                PoseStack poseStack,
                float partialTick,
            ...
            super(entity, poseStack, partialTick);
            this.renderer = renderer;
            this.packedLight = packedLight;
        }

        private AvatarRenderer getRenderer() {
            return renderer;
        }
    ...
    private static void init() {
        dshieldResilienceKey = KeyBindingHelper.registerKeyBinding(createDShieldResilienceKeyMapping());
        // Process this before Minecraft handles its own key bindings.  The default
        // middle mouse binding is also used by pick-block, and whichever binding
        // is registered last receives the click event.
        ClientTickEvents.START_CLIENT_TICK.register(ModCombatClientEvents::sendDShieldResilienceRequests);
        HudRenderCallback.EVENT.register(ModCombatClientEvents::onRenderGuiPost);
        registerItemProperties();
        registerBetterCombatAttackStartListener();
        registerBetterCombatAttackHitListener();
    }
    private static void sendDShieldResilienceRequests(Minecraft client) {
        ...
        // KeyMapping keeps only one mapping per input. If vanilla's pick-block
        // mapping won the middle-mouse registration race, consume that mapping
        // instead of losing the dshield action.
        if (isMiddleMouseBinding(dshieldResilienceKey)) {
            while (client.options.keyPickItem.consumeClick()) {
                clicked = true;
            }
        }
        if (clicked) {
            ClientPlayNetworking.send(new DShieldResiliencePayload());
        }
    }

    private static boolean isMiddleMouseBinding(KeyMapping keyMapping) {
        return "key.mouse.middle".equals(keyMapping.saveString());
    }
    private static KeyMapping createDShieldResilienceKeyMapping() {
        ...
        return new KeyMapping(
                ...
                key.getType(),
                key.getValue(),
                KeyMapping.Category.register(Identifier.fromNamespaceAndPath(FiverkasWeapons.MODID, "fweapons"))
        );
    }
    ...
    private static void registerItemProperties() {
        // 1.21.11 moved item model predicates to data-driven item model properties.
        // Keep startup working; model JSON can be migrated separately.
    }
    private static void onRenderPlayerPre(PlayerRenderPreEvent event) {
    ...
    private static void onRenderPlayerPost(PlayerRenderPostEvent event) {
        if (MKOPI_SHAKE_RENDER_PUSHED.remove(event.getEntity().getId())) {
            event.getPoseStack().popPose();
        }
        renderResilienceOutline(event);
    }
    private static void renderResilienceOutline(PlayerRenderPostEvent event) {
        ...
        outlineBuffer.setColor(0xFF000000 | (RESILIENCE_OUTLINE_RED << 16) | (RESILIENCE_OUTLINE_GREEN << 8) | RESILIENCE_OUTLINE_BLUE);
        Identifier texture = clientPlayer.getSkin().body().texturePath();
    ...
    private static RenderType resilienceOutlineRenderType(Identifier texture) {
    ...
    private static void renderPlayerModelForResilienceOutline(
        ...
        boolean shouldSit = player.isPassenger() && player.getVehicle() instanceof LivingEntity;
    ...
    private static void setupResilienceOutlineRotations(
        ...
        } else if (isEntityUpsideDown(player)) {
    ...
    private static boolean isEntityUpsideDown(AbstractClientPlayer player) {
    ...
    private static void applyMkopiRenderShake(PlayerRenderPreEvent event) {
        Player player = event.getEntity();
        ...
        try {
            ...
            try {
                ...
                if (attackCtor != null) {
                    int lastIndex = 5;
                    int selectedSlot = 0;
                    try { selectedSlot = player.getInventory().getSelectedSlot(); } catch (Throwable ignored) {}
                    Object attackReq;
    ...
    private static void onRenderGuiPost(GuiGraphics guiGraphics, DeltaTracker tickCounter) {
                    ...
                    (proxy, method, args) -> {
                        ...
                        if (attackHand != null) {
                            ...
                            if (isBayonetImpactAttack(attackInfo)) {
                                ClientPlayNetworking.send(new BayonetComboAttackPayload());
                            ...
                            if (isBayonetGunshotAttack(attackInfo)) {
                                ...
                                ClientPlayNetworking.send(new BayonetMuzzleFlashPayload());
                            }
