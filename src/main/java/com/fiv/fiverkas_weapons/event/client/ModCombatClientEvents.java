...
public final class ModCombatClientEvents {
    ...
    private static void onKeyInput(InputEvent.Key event) {
        if (event.getAction() == InputConstants.PRESS) {
            sendDShieldResilienceRequestsForKey(event.getKeyEvent());
        }
    }
    private static void onMouseInputPost(InputEvent.MouseButton.Post event) {
        if (event.getAction() == InputConstants.PRESS) {
            sendDShieldResilienceRequestsForMouse(event.getMouseButtonInfo());
        }
    }
    private static void sendDShieldResilienceRequestsForKey(net.minecraft.client.input.KeyEvent keyEvent) {
        if (dshieldResilienceKey == null) {
            return;
        }
        if (dshieldResilienceKey.matches(keyEvent)) {
    ...
    private static void sendDShieldResilienceRequestsForMouse(net.minecraft.client.input.MouseButtonInfo buttonInfo) {
        if (dshieldResilienceKey == null) {
            return;
        }
        if (dshieldResilienceKey.getKey().getType() == InputConstants.Type.MOUSE
                && dshieldResilienceKey.getKey().getValue() == buttonInfo.button()) {
            sendDShieldResilienceRequest();
            clearQueuedDShieldResilienceClicks();
            return;
        }
    ...
    private static void sendDShieldResilienceRequest() {
        ClientPacketDistributor.sendToServer(new DShieldResiliencePayload());
    }
    ...
    private static KeyMapping createDShieldResilienceKeyMapping() {
        ...
        return new KeyMapping(
                ...
                key.getType(),
                key.getValue(),
                KeyMapping.Category.register(ResourceLocation.fromNamespaceAndPath(FiverkasWeapons.MODID, "fweapons"))
        );
    }
    ...
    private static void registerItemProperties() {
        // Item model predicates were replaced by data-driven item models in 1.21.10.
    }
    private static void onRenderPlayerPre(RenderPlayerEvent.Pre event) {
        Player player = renderedPlayer(event);
        if (player == null) {
            return;
        }
        if (player.hasEffect(ModEffects.CERULEAN_SHROUD)) {
            event.setCanceled(true);
            return;
        }
        if (isUsingMkopi(player)) {
    ...
    private static void onRenderPlayerPost(RenderPlayerEvent.Post event) {
        Player player = renderedPlayer(event);
        if (player != null && MKOPI_SHAKE_RENDER_PUSHED.remove(player.getId())) {
            event.getPoseStack().popPose();
        }
        if (player != null && player.hasEffect(ModEffects.RESILIENCE)) {
            event.getRenderState().outlineColor = 0xFF_A0_20_F0;
        }
    }
    private static Player renderedPlayer(RenderPlayerEvent<?> event) {
        Minecraft minecraft = Minecraft.getInstance();
        if (minecraft.level == null) {
            return null;
        }
        return minecraft.level.getEntity(event.getRenderState().id) instanceof Player player ? player : null;
    }
    /*
    ...
    */
                    ...
                    (proxy, method, args) -> {
                        ...
                        if (attackHand != null) {
                            ...
                            if (isBayonetImpactAttack(attackInfo)) {
                                ClientPacketDistributor.sendToServer(new BayonetComboAttackPayload());
                            ...
                            if (isBayonetGunshotAttack(attackInfo)) {
                                ...
                                ClientPacketDistributor.sendToServer(new BayonetMuzzleFlashPayload());
                            }
    ...
    private static void spawnBayonetGunshotParticles(LocalPlayer player) {
        ...
        RandomSource random = player.getRandom();
        player.level().addParticle(net.minecraft.core.particles.ColorParticleOption.create(ParticleTypes.FLASH, 0xFFFFFFFF), x, y, z, 0.0, 0.0, 0.0);
